...
describe('lib/UIElements', () => {
    ...
    describe('createModal', () => {
        ...
        it('should accept custom className', () => {
            ...
            expect(createModal({ className: 'custom' }).className).toBe(`${MODAL_CLASSNAME} custom`);
        });
        ...
        it('should use suffix override', () => {
            const selector = 'p:not([class])';
            expect(createModal().querySelector(selector)).not.toBe(null);
            expect(createModal({ suffix: null }).querySelector(selector)).toBe(null);
            expect(createModal({ suffix: 'Something Unexpected' }).querySelector(selector).textContent).toBe('Something Unexpected');
        });
        it('should use quickfixes override', () => {
            const selector = `p.${QUICKFIX_CLASSNAME}`;
            expect(createModal().querySelector(selector)).not.toBe(null);
